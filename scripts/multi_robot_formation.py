...
follower_2 = Articulation("/robile_follower_2/base_link")

leader.initialize()
follower_1.initialize()
follower_2.initialize()

fleet = [leader, follower_1, follower_2]

# =====================================================
# 2. Goal Pose & Logging Setup
# =====================================================
...
csv_path = os.path.expanduser(
    "~/isaac_projects/robile_assignment/scripts/isaacsim_fleet_data_n3.csv"
)
csv_data = [["frame", "leader_x", "leader_y", "f1_x", "f1_y", "f2_x", "f2_y"]]

# NEW: Warm-up variables to let physics settle
WARM_UP_FRAMES = 300
...
def on_update(event):
    ...
    # Phase 1: The Warm-Up
    # Let the robots fall and settle for the first 60 frames
    ...
    # Phase 2: Drive and Log Data
    l_pos, _ = leader.get_world_poses()
    f1_pos, _ = follower_1.get_world_poses()
    f2_pos, _ = follower_2.get_world_poses()

    current_x = l_pos[0][0]

    csv_data.append(
        [
            logging_frame_counter,
            l_pos[0][0],
            l_pos[0][1],
            f1_pos[0][0],
            f1_pos[0][1],
...
subscription = stream.create_subscription_to_pop(on_update, name="FleetNavigation")
print("Multi-Robot Navigation initializing...")
